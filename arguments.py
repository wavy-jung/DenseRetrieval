...
class TrainingArguments:
    ...
    per_device_train_batch_size: int = field(
        default=4, metadata={"help": "train batch size (num queries per batch)"}
        )
    ...
    device: str = field(
        default=avail_device, metadata={"help": "train in which device"}
        )
        
    warmup_steps: int = field(
        default=0, metadata={"help": "steps for learning rate warmup"}
        )
